            ...
            if (OVRInput.IsControllerConnected(OVRInput.Controller.Hands))
            {
                ...
                else
                {
                            ...
                            Vector3.Distance(leftThumbFingerTip.position, rightThumbFingerTip.position) < fingerMinimumDistance)
                    {
                        ...
                        if(rightHand.IsHighConfidence && leftHand.IsHighConfidence)
                        {
                            if (Vector3.Distance(leftIndexFingerTip.position, leftThumbFingerTip.position) < fingerMinimumDistance &&
                            Vector3.Distance(righttIndexFingerTip.position, rightThumbFingerTip.position) < fingerMinimumDistance)
                            {
