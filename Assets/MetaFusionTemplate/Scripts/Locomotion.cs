...
namespace Chiligames.MetaFusionTemplate
{
    public class Locomotion : MonoBehaviour
    {
        ...
        private void ShowPointerController()
        {
            ...
            if (Physics.Raycast(startingPoint.position, startingPoint.forward, out hit, 10, teleportLayermask))
            {
        ...
        private void ShowPointerHandTracking(Vector3 _startPoint, Vector3 _direction)
        {
            ...
            if (Physics.Raycast(_startPoint, _direction, out hit, 10, teleportLayermask))
            {
                if(teleportPoint.enabled == false)
                {
                    teleportPoint.enabled = true;
                ...
                var _teleportPoint = hit.point;
                teleportPoint.transform.position = Vector3.Lerp(teleportPoint.transform.position, hit.point, 0.35f);
        ...
        private void Teleport()
        {
            if (ableToTeleport)
            {
                OVRRig.transform.position = new Vector3(teleportPoint.transform.position.x, teleportPoint.transform.position.y, teleportPoint.transform.position.z) - new Vector3(centerEyeAnchor.transform.localPosition.x, 0, centerEyeAnchor.transform.localPosition.z);
            }
        }
